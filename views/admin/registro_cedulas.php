        ...
        <main class="main-content">
            ...
            <div class="card">
                <div class="card-header">
                    <h2>Agregar Nueva Cédula Autorizada</h2>
                </div>
                <div class="card-body">
                    <form method="POST" action="<?= BASE_URL ?>admin/agregar-cedula" class="form-inline">
                        <div class="form-group">
                            <label for="cedula">Cédula:</label>
                            <input type="text" id="cedula" name="cedula" required placeholder="Ingrese número de cédula" pattern="[0-9]*" inputmode="numeric">
                        </div>
                        <div class="form-group">
                            <label for="nombre_referencia">Nombre de Referencia:</label>
                            <input type="text" id="nombre_referencia" name="nombre_referencia" required placeholder="Nombre o descripción">
                        </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h2>Cédulas Autorizadas Registradas</h2>
                </div>
                <div class="card-body">
                    <div class="usuarios-list">
                        <div class="search-container">
                            <div class="search-input-wrapper">
                                ...
                                <input type="text" class="form-control" data-table-search="tabla-cedulas" placeholder="Buscar por cédula o nombre...">
                            </div>
                        </div>
                        <table class="table" id="tabla-cedulas">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Cédula</th>
                            ...
                            <tbody>
                                <?php if (empty($cedulas)): ?>
                                    <tr>
                                        <td colspan="5" style="text-align: center;">No hay cédulas registradas</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($cedulas as $cedula): ?>
                                        <tr>
                                            <td><?= $cedula->getId() ?></td>
                                            <td><?= htmlspecialchars($cedula->getCedula()) ?></td>
                                            <td><?= htmlspecialchars($cedula->getNombreReferencia()) ?></td>
                                            <td><?= date('d/m/Y H:i', strtotime($cedula->getFechaRegistro())) ?></td>
                                            <td>
                                                <form method="POST" action="<?= BASE_URL ?>admin/eliminar-cedula/<?= $cedula->getId() ?>" style="display: inline;">
                                                    <button type="submit" class="btn btn-sm" style="background-color: #ef4444; color: white;" data-confirm-delete data-item-name="<?= htmlspecialchars($cedula->getCedula()) ?>">Eliminar</button>
                                                </form>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
